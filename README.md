...
- **A PHP + MySQL host** — e.g. InfinityFree, 000webhost, Hostinger, or a
  small VPS / Railway / Render with PHP 8 and MySQL. Deploy steps there:
  1. Upload the project files.
  2. Create a database and import `schema.sql`.
  3. Set DB credentials in `config.php` (or via `DB_*` env vars).
  4. Populate the catalog: run `php import_bhoi.php` (it fetches the archive
     and writes the PDFs/solutions into `uploads/`).
  5. Make sure `uploads/` is writable by the web server.

The repo deliberately does **not** commit the database or the uploaded files
