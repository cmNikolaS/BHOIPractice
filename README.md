...
**GitHub Pages cannot run this app.** Pages only serves static HTML/JS/CSS,
while this is a PHP + MySQL application. GitHub is great for storing the *code*,
but it will not *execute* it — the old `Deploy to Pages` workflow has been
removed because it could only ever fail here.

To actually run the site, use one of:

- **Locally** — double-click `preview.bat` (see *Quick preview* above).
- **Fly.io** — recommended for a public URL (see below).
- **Any PHP + MySQL host** — InfinityFree, Hostinger, a VPS, etc.: upload the
  files, import `schema.sql`, set `DB_*` in `config.php`, run `php import_bhoi.php`,
  and make `uploads/` writable.

### Deploy to Fly.io  →  `https://<app>.fly.dev`

The repo ships a `Dockerfile` + `fly.toml` for a **single, self-contained
container**: PHP 8.2 + Apache, with the database as a **SQLite file on a
persistent volume** (no separate DB service). The whole BHOI catalog (PDFs +
solutions) is baked into the image at build time, so the site comes up
populated and fast.
...
# 2. Pick a unique app name (edit `app = "..."` in fly.toml), then:
fly apps create <your-app-name>

# 3. Create the persistent volume (same region as fly.toml's primary_region)
fly volumes create bhoi_data --size 1 --region fra

# 4. Deploy — builds the image (downloads the archive) and launches it
fly deploy

# 5. Open it
fly open                       # -> https://<your-app-name>.fly.dev
```

The container reads `DB_DRIVER=sqlite`, `DB_SQLITE_PATH=/data/app.sqlite`,
`UPLOAD_DIR=/data/uploads` from `fly.toml` (all on the `/data` volume, which
survives restarts and redeploys). Admin login is `admin` / `admin123` —
**change it** (see above). To rebuild the catalog later:
`fly ssh console` then `cd /var/www/html && php import_bhoi.php`.

The repo deliberately does **not** commit the database or uploaded files
(`uploads/` contents are git-ignored); they're regenerated by `import_bhoi.php`,
so a fresh clone stays small.
